...
int main2(int argc, const char * argv[]) {
    ...
    kd_regtype kdr = { 0 };
    kdr.type = 0x00200000U; // KDBG_VALCHECK
    kdr.value1 = (6 << 16) | (8 << 8); // KDBG_EVENTID(DBG_PERF, PERF_KPC, PERF_KPC_DATA_THREAD)
    ...
    while (buf_hdr) {
        ...
        for (kd_buf *buf = buf_cur, *end = buf_cur + count; buf < end; buf++) {
            ...
            u32 cls = (debugid >> 16) & 0xFF;
            u32 subcls = (debugid >> 8) & 0xFF;
            u32 code = (debugid) & 0xFF;

            // keep only thread PMC data
            if (cls != 6) continue;    // DBG_PERF
            if (subcls != 6) continue; // PERF_KPC
            if (code != 8) continue;   // PERF_KPC_DATA_THREAD
    ...
    for (kd_buf *buf = buf_hdr; buf < buf_cur; buf++) {
        u32 func = buf->debugid & 0xff000000U;
        if (func != 0) continue; // only start
        ...
        if (ci < counter_count) {
            // counter count larger than 4, values are split into multiple buffer entries
            for (kd_buf *buf2 = buf + 1; buf2 < buf_cur; buf2++) {
                u32 tid2 = (u32)buf2->arg5;
                if (tid2 != tid) break;
                u32 func2 = buf2->debugid & 0xff000000U;
                if (func2 == 0) break;
